...
class SaleOrder(models.Model):
    ...
    bayan = fields.Char(string="Bayan", tracking=True)
    shipping_line = fields.Many2one('shipping.sale.order', string="Shipping line")
    landing_place = fields.Many2one('sea.port')
    order_status_id = fields.Many2one('fmt.order.status', string="Order Status", tracking=True)
    ...
    def create(self, values):
        ...
        print("values", values)
        if values['bill_of_lading']:
            res.message_post(body="Bill of lading : %s" % values['bill_of_lading'])
        if values['bayan']:
            res.message_post(body="Bayan : %s" % values['bayan'])
        if values['order_status_id']:
            res.message_post(body="Order Status : %s" % res.order_status_id.name)
        return res
